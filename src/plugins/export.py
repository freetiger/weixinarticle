# -*- coding: utf-8 -*-
'''
优化xadmin的导出功能，修复bug：Django1.7b版本中HttpResponse中 移除了参数mimetype='application/json' 这样的写法，改为content_type='application/json'。
list_export_fixed赋值('xlsx', 'xls', 'csv', 'xml', 'json')，设置导出类型，默认不使用此导出功能。xadmin的导出功能是默认使用的。
'''
import StringIO
import datetime
import sys
...
class ExportMenuPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        self.list_export_fixed = [
            f for f in self.list_export_fixed
            if (f != 'xlsx' or has_xlsxwriter) and (f != 'xls' or has_xlwt)]
        
    def block_top_toolbar(self, context, nodes):
        if self.list_export_fixed:
            context.update({
                ...
                'form_params': self.admin_view.get_form_params({'_do_': 'export_fixed'}, ('export_type',)),
                'export_types': [{'type': et, 'name': self.export_names[et]} for et in self.list_export_fixed],
            })
...
class ExportPlugin(BaseAdminPlugin):
    ...
    def init_request(self, *args, **kwargs):
        return self.request.GET.get('_do_') == 'export_fixed'
